...
RgbVideoDataLayer<Dtype>::~RgbVideoDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
float RgbVideoDataLayer<Dtype>:: random(float start, float end)
{
...
template <typename Dtype>
int RgbVideoDataLayer<Dtype>::myrandomdata (int i) { return caffe_rng_rand()%i;}

template <typename Dtype>
void RgbVideoDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (crop_size > 0) {
    ...
    this->transformed_data_.Reshape(1, cv_img.channels() , crop_size, crop_size);
  } else {
    top[0]->Reshape(batch_size, channels, height, width);
    this->prefetch_data_.Reshape(batch_size, channels, height, width);
    this->transformed_data_.Reshape(1, cv_img.channels() , height, width);
  }
...
template <typename Dtype>
void RgbVideoDataLayer<Dtype>::ShuffleImages() {
  ...
  std::random_shuffle(tnum.begin(), tnum.end(), RgbVideoDataLayer<Dtype>::myrandomdata);
...
template <typename Dtype>
void RgbVideoDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; item_id += pairsize) {
    ...
    int video_frames = lines_[lines_id_].second;
    lines_id_ ++;
    ...
    for(int i = 0; i < pairsize; i ++)
    {
        ...
        cv::Mat cv_img = ReadImageToCVMat(root_folder + lines_[lines_id_ + id].first,
            new_height, new_width, is_color);
        // debug
...
INSTANTIATE_CLASS(RgbVideoDataLayer);
REGISTER_LAYER_CLASS(RgbVideoData);
